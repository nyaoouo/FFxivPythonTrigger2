...
class Stage1:
    ...
    def deal(self, craft, prev_skill=None):
        ...
        if prev_skill == "专心加工:fail":
            self.count2 += 1
        if self.count > 6 or self.count2 > 2 or craft.current_cp < 250:
            return "terminate"
        if craft.status == "長持続":
            if not process_finish and "崇敬" not in craft.effects:
                return "崇敬"
            if craft.current_cp > 350:
                if "俭约" not in craft.effects:
                    return "俭约"
                if "掌握" not in craft.effects:
                    return "掌握"
        if craft.status == "高品质":
            ...
            return "秘诀"
        if craft.status == "高效" and craft.current_cp >= 300:
            if "掌握" not in craft.effects:
                return "掌握"
            if craft.recipe.max_durability - craft.current_durability >= 30:
                return "精修"
            if "俭约" not in craft.effects:
                return "长期俭约"
