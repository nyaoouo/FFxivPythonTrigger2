...
def allowSkills(craft):
    ...
    if (
        "改革" in craft.effects
        or craft.current_cp < 60
        or craft.status.name in SpecialStatus
    ):
        to_add = list()
        ...
        for skill in to_add:
            if craft.current_durability > craft.get_skill_durability(
                skill
            ) and craft.current_cp >= craft.get_skill_cost(skill):
                ans.append(skill)
    if not (
...
def try_solve(craft: Craft, timeLimit=None):
    ...
    while queue:
        ...
        if (
            best_start_with is not None
            and len(t_history) > 8
            and t_history[:best_start_with_l] != best_start_with
        ):
            continue
        ...
        for skill in skills:
            try:
                ...
                debug(
                    "solver astar", "error at testing skill %s:\n%s" % (skill, t_craft)
                )
            ...
            if skill != "比尔格的祝福":
                if tt_craft.current_durability >= 1 and (
                    best_un_finish is None
                    or tt_craft.current_quality > best_un_finish[0].current_quality
                ):
...
class Stage3:
    def __init__(self):
        ...
        self.blueprint = min(3,sum([i.count for i in api.XivMemory.inventory.get_item_in_pages_by_key(28724, "backpack")]))
    ...
    def deal(self, craft, prev_skill=None):
        if (
            self.queue[0] == "比尔格的祝福"
            and craft.status != "高品质"
            and self.blueprint > 0
            and craft.recipe.max_quality - craft.current_quality
            > craft.get_skill_quality("比尔格的祝福")
        ):
            self.blueprint -= 1
            return "设计变动"
